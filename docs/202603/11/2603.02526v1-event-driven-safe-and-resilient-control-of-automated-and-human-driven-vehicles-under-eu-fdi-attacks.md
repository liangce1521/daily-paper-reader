...
title_zh: 指数无界虚假数据注入（EU-FDI）攻击下自动驾驶与人类驾驶车辆的事件驱动安全韧性控制
authors: "Yi Zhang, Yichao Wang, Wei Xiao, Mohamadamin Rajabinezhad, Shan Zuo"
date: 2026-03-03
pdf: "https://arxiv.org/pdf/2603.02526v1"
tags: ["query:eco-ai"]
score: 7.0
evidence: 混合交通环境下自动驾驶车辆的控制
tldr: 针对混合交通环境下联网自动驾驶车辆（CAV）面临的指数无界虚假数据注入（EU-FDI）攻击及人类驾驶车辆（HDV）行为不确定性，本文提出一种事件驱动的安全弹性控制框架（EDSR）。该框架结合控制屏障函数（CBF）、控制李雅普诺夫函数（CLF）与自适应弹性控制，并利用数据驱动方法估计HDV行为。在换道场景下的仿真验证表明，该方法能有效降低计算负担，在对抗环境下实现无碰撞行驶和稳定的速度调节。
selection_source: fresh_fetch
motivation: 现有的弹性控制往往忽视碰撞规避，而安全控制方法通常缺乏对对抗性攻击（如EU-FDI）的抵御能力。
method: 提出一种集成事件驱动CBF-CLF、自适应弹性控制及数据驱动HDV行为估计的综合控制框架。
result: 仿真结果显示，该框架在面临加速度输入攻击和不可预测的HDV行为时，能成功实现安全换道和速度稳定。
conclusion: 该研究为混合交通流中CAV的安全与弹性控制提供了高效且鲁棒的解决方案，平衡了实时安全性与计算效率。
---

## 摘要
本文研究了在混合交通环境下，联网自动驾驶车辆（CAVs）在不确定动力学和指数无界虚假数据注入（EU-FDI）攻击下与人类驾驶车辆（HDVs）交互时的安全韧性控制。这些攻击对安全关键型应用构成了严重威胁。虽然韧性控制策略可以减轻对抗性影响，但它们往往忽略了避障要求。相反，专注于安全的方法往往假设处于名义运行条件，缺乏对对抗性输入的韧性。为了应对这些挑战，我们提出了一个控制框架，将事件驱动的控制屏障函数（CBFs）和控制李雅普诺夫函数（CLFs）与自适应攻击韧性控制相结合。该框架进一步结合了 HDV 行为的数据驱动估计，以确保针对 EU-FDI 攻击的安全性和韧性。具体而言，我们关注在存在不可预测的 HDV 以及针对加速度输入的 EU-FDI 攻击的情况下，CAV 的换道操作。事件驱动方法在保持实时安全保障的同时降低了计算负荷。仿真结果（包括与缺乏韧性的纯事件驱动方法的对比）验证了所提出的 EDSR 框架在实现无碰撞操作、稳定速度调节以及对抗条件下的韧性运行方面的有效性和鲁棒性。
