...
## 摘要
我们从 Wasserstein 几何的角度研究了针对连续动作、熵正则化强化学习的策略梯度方法。从 Wasserstein 近端更新出发，我们通过一种算子分裂方案推导出 Wasserstein 近端策略梯度 (WPPG)，该方案交替进行最优传输更新和由高斯卷积实现的热步（heat step）。这种表述避免了评估策略的对数密度及其梯度，使得该方法可以直接应用于以推前映射（pushforward maps）形式指定的具有强表达能力的隐式随机策略。我们为 WPPG 建立了全局线性收敛率，涵盖了精确策略评估以及具有受控近似误差的 Actor-Critic 实现。实验表明，WPPG 易于实现，并在标准连续控制基准测试中取得了具有竞争力的性能。
