...
tags: ["query:eco-ai"]
score: 7.0
evidence: 用于离线强化学习和轨迹一致性的扩散规划器
tldr: 针对扩散规划器在离线强化学习中因价值引导导致轨迹与环境动力学不一致的问题，本文提出SAGE方法。SAGE通过自监督学习JEPA编码器和潜空间预测器，在推理阶段利用潜空间预测误差作为能量分数对候选轨迹进行重排序。该方法无需环境交互或重新训练策略，显著提升了扩散规划器在多种任务中的性能和鲁棒性。
selection_source: fresh_fetch
motivation: 扩散规划器在离线强化学习中常因价值引导而选择得分高但动力学不一致的脆弱轨迹。
method: 提出SAGE方法，利用JEPA架构在潜空间评估轨迹的一致性，并结合预测误差与价值估计进行动作选择。
result: 在运动、导航和操作等多个基准测试中，SAGE显著提高了现有扩散规划器的性能和鲁棒性。
conclusion: SAGE作为一种无需重训的推理端重排序插件，能有效过滤动力学不一致的计划，增强扩散规划的可靠性。
---
