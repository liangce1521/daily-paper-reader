...
tags: ["query:eco-ai"]
score: 8.0
evidence: 用于自动驾驶场景生成的LLM和扩散模型
tldr: 针对自动驾驶安全关键场景稀缺且现有生成方法在可控性与真实性上存在不足的问题，本文提出 AnchorDrive 框架。该框架结合 LLM 的语义控制能力与扩散模型的真实分布建模能力，通过两阶段流程——LLM 闭环仿真生成语义轨迹及锚点引导的扩散模型轨迹重构，实现了高真实感且语义可控的安全关键场景生成，在 highD 数据集上表现优异。
selection_source: fresh_fetch
motivation: 现有的安全关键场景生成方法难以同时兼顾自然语言指令的可控性和生成轨迹的真实性。
method: 提出两阶段框架，先利用 LLM 结合反馈机制生成语义可控轨迹，再提取关键锚点引导扩散模型进行高真实感轨迹重构。
result: 在 highD 数据集上的实验证明，AnchorDrive 在场景危险程度、真实性以及用户意图的可控性方面均优于现有方法。
conclusion: 结合大语言模型与扩散模型的优势，是生成高质量、可控且符合真实分布的安全关键驾驶场景的有效途径。
---
