...
motivation: 现有的EVRPTW基准数据集通常是静态的且无法保证实例的可解性，限制了对基于学习的路由模型的可靠评估。
method: 开发了一个集成几何特征、自适应能量容量缩放和充电站布局的参数化生成器，并引入快速可行性筛选机制。
result: 成功实现了可生成5至500个客户规模的多样化实例，并能系统地过滤掉不可解的实例，确保了数据的有效性。
conclusion: SynthCharge为神经路由和数据驱动方法提供了动态的基准测试框架，有助于系统地评估算法的鲁棒性。
---
